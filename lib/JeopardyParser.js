...
var JeopardyParser = {
	...
	parseCategoryComments: function($, $category) {
		let categoryComments = $category.find(".category_comments").text().trim();
		// Category comments seem to be in the form "(Alex: whatever the comment is)"
		// So let's naively chop off the front and back
		const start = "(Alex: ".length;
		const end = -1 * ")".length;
		return categoryComments.slice(start, end);
	},
	...
	parseJeopardySubgame: function($, jeopardyDiv, dollarAmountDelta){
		...
		game.categories = [];
		//parse categories
		jeopardyDiv.find(".category").each((index, element) => {
			const categoryTitle = $(element).find(".category_name").text().trim();
			const categoryComments = this.parseCategoryComments($, $(element));
			const newCategory = {
				name: categoryTitle,
				clues: [],
				categoryComments
	...
	parseFinalJeopardy: function($){
		...
		const media = this.parseMedia($, $(".final_round .clue"));
		return {
			categories: [{
				name: categoryName,
				question,
